...
def write_snapshots():
    """Emit docs/snapshots.json: one compact file with, per scheme, its latest NAV and its
    NAV on-or-before 31-Dec of every year 2006..last year. Lets browser tools load
    20 years of year-end data for every fund in ONE request instead of one per fund.
    Shape: {"v":1,"generated_ist":..,"years":[2006..],
            "f":{code:[name, latestDate "DD-MM-YYYY", latestNav, [yearEndNav|0,...]]}}"""
    this_year = datetime.now(IST).year
    years = list(range(2006, this_year))
    ye_dates = [datetime(y, 12, 31) for y in years]
    cutoff = datetime(2006, 1, 1)
    f = {}
    for fn in os.listdir(HIST_DIR):
        if not fn.endswith(".json"):
            continue
        h = load_json(os.path.join(HIST_DIR, fn), None)
        if not h or not h.get("points"):
            continue
        pts = []
        for d, v in h["points"].items():
            try:
                dt = datetime.strptime(d, "%d-%m-%Y"); nv = float(v)
        ...
        pts.sort()
        if pts[-1][0] < cutoff:
            continue  # died before the analysis window
        ye, i = [], 0
        last = pts[-1][0]
        for t in ye_dates:
            if t >= last:
                break  # latest point already covers this year-end
            while i < len(pts) and pts[i][0] <= t:
                i += 1
            ye.append(float(f"{pts[i-1][1]:.6g}") if i else 0)
        while ye and ye[-1] == 0:
            ye.pop()
        f[fn[:-5]] = [h.get("meta", {}).get("scheme_name", ""),
                      pts[-1][0].strftime("%d-%m-%Y"), float(f"{pts[-1][1]:.6g}"), ye]
    out = {"v": 1, "generated_ist": datetime.now(IST).isoformat(), "years": years, "f": f}
    with open(os.path.join(ROOT, "docs", "snapshots.json"), "w") as fh:
        json.dump(out, fh, separators=(",", ":"))
    print(f"  wrote snapshots.json for {len(f)} schemes")
